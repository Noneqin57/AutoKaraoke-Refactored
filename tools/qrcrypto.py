...
本文件的核心 DES 算法代码来自开源项目 LDDC（chenmozhijin/LDDC，
https://github.com/chenmozhijin/LDDC ），遵循 GPL-3.0-only 许可：

    SPDX-FileCopyrightText: Copyright (C) 2024-2025 沉默の金 <cmzj@cmzj.org>
    SPDX-License-Identifier: GPL-3.0-only
    参考原C#代码: https://github.com/WXRIW/QQMusicDecoder

使用方式（见 collect_test_data.py）：
    解密 `GetPlayLyricInfo` 返回的加密 hex 字符串 -> 得到 QRC/纯文本歌词。

注意：
- QQ音乐歌词 API 的 `lyric` 字段不是标准 3DES-ECB，而是经过自定义位置换
  DES 的 3DES 变体，因此不能用 pycryptodome 等标准库直接解密。
- 此模块为独立文件，不依赖 PySide6 / LDDC 包本体。
"""
import zlib
...
# QQ音乐 QRC 云端加密使用的 3DES 密钥
QRC_KEY = b"!@#)(*$%123ZXC!@!@#)(NHL"


def qrc_decrypt(encrypted_qrc: str) -> str:
    """解密 QQ音乐 `GetPlayLyricInfo` 返回的加密歌词。

    Args:
        encrypted_qrc: API 返回的十六进制字符串（`lyric` / `trans` / `roma` 字段）。

    Returns:
        解密后的纯文本歌词（QRC XML 格式，或纯文本/LRC 格式）。

    Raises:
        ValueError: 解密失败（密文为空 / zlib 解压失败等）。
    """
    if not encrypted_qrc or not encrypted_qrc.strip():
    ...
    # 以 8 字节为单位迭代（自定义 DES 每次只处理前 8 字节，等价于 ECB 分块）
    ...
    try:
        ...
        raise ValueError(f"QRC 解密失败: {e}") from e